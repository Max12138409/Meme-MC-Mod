...
local TAG_missile_tudou = "<size=300%><sprite name=mod_missile_tudou></size>"--土豆
local TAG_missile_shazi = "<size=300%><sprite name=mod_missile_shazi></size>"--沙子
local TAG_missile_jinli = "<size=300%><sprite name=mod_missile_jinli></size>"--金粒
local TAG_missile_keyideshazi = "<size=300%><sprite name=mod_missile_keyideshazi></size>"--可疑的沙子
local TAG_missile_furou = "<size=300%><sprite name=mod_missile_furou></size>"--腐肉
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Api:RegisterChoice({
        ...
        options = {
            {
                flavor = "说自己要去下界请求他的帮助 "..TAG_relic_xjhj_d,
...
--自定义属性
--鸡计数器
local ji_quest_max = 150
Api:RegisterAttribute({
    id = "JS_ji",
    display_name = "孵化进度",
    description = "孵化进度达到100时随机孵化一枚蛋",
    icon = DCEI.Texture("Missile_Ji"),
...
Api:RegisterRelic("Relic_RenWu", {
        id = "Relic_RenWu",
        display_name = "下界还是末地",
        description = "前往下界还是末地？\n".."任务：获得"..TAG_relic_zhenzhu.."或"..TAG_relic_dahuoshi,
        rarity = "legendary",
        icon = DCEI.Texture("Relic_RenWu"),  
        flavor_text = "选择你的道路(选择之后其他任务线将会关闭)",
        ...
        combat_unit:RegisterOnCombatStartCallback(name, function(level,target)
            if combat_unit:HasRelic("Relic_DaHuoShi") then
                       combat_unit:ApplyRelic("Relic_RenWu_QWXJ")
...
Api:RegisterRelic("Relic_CSM", {
        id = "Relic_CSM",
        display_name = "下界传送门",
        description ="战斗开始时从下界随机召唤1~3只"..TAG_missile_zhuren.."协助你作战，获得".."<color=#47b05d>".."3</color>"..Api.GameMechanicTags.TAG.armor.."<color=#47b05d>".."1</color>"..Api.GameMechanicTags.TAG.attack..
...
Api:RegisterRelic("Relic_YanJing", {
        ...
        combat_unit:RegisterOnCombatStartCallback(name, function(level,target)
            if combat_unit.run_instance.wave == 13 then
                --添加子弹--战斗结束后不保留
                combat_unit.Attack:AddEmojiToDeck("Missile_YanJing", 1)
                --播放添加子弹的动画
                combat_unit:PlayAwardEmojiAnimation("Missile_YanJing", 1)
                ...
                combat_unit.run_instance:ModifySavedPerkDataEmojis("Missile_YanJing", deck_size)
            end    
        end)
        end)
...
Api:RegisterRelic("Relic_ZuanShiJian", {
        ...
        combat_unit:RegisterOnCombatStartCallback(name, function(level,target)
                for i = 1, level do
                --添加子弹--战斗结束后不保留
                combat_unit.Attack:AddEmojiToDeck("Missile_ZSJ", 1)
                --播放添加子弹的动画
                combat_unit:PlayAwardEmojiAnimation("Missile_ZSJ", 1)
                end
        end)
        end,false)
...
Api:RegisterRelic("Relic_MianBao", {
    ...
    icon = DCEI.Texture("Missile_MianBao"),
    rarity = "common",
    ...
    behavior = {
        description = "能量恢复",
        buff_bar_icon = DCEI.Texture("Missile_MianBao"),
        modify_attributes = {
            health_per_second = 0,
              },
        is_positive = true,
    ...
    combat_unit.Attack:RegisterOnMissileImpactCallback(name, function(level, attack_data, criminal, victim)
        if attack_data.missile_id == "Missile_MianBao" then
            for i = 1, level do
    ...
    combat_unit:RegisterOnCombatStartCallback(name, function(level,target)
        if combat_unit.run_instance.wave == 8 then
            --添加子弹--战斗结束后不保留
            combat_unit.Attack:AddEmojiToDeck("Missile_MianBao", 1)
            --播放添加子弹的动画
            combat_unit:PlayAwardEmojiAnimation("Missile_MianBao", 1)
            --获取弹夹里子弹数量--临时添加的子弹也算在里面
            local deck_size = #combat_unit.Attack:GetEmojiDeck()
            --战斗结束后永久添加子弹(deck_size差不多就是弹夹里最后一个子弹的位置,之前超过这个数会卡死,要用这个的话得注意下顺序
            combat_unit.run_instance:ModifySavedPerkDataEmojis("Missile_MianBao", deck_size)
        end    
    end)
    end)
...
Api:RegisterPerk("Perk_MuTi", {
        id = "Perk_MuTi",
        display_name = "母体链接",
        description = "蜘蛛眼命中敌人时有".."<color=#47b05d>".."15%</color>".."几率发射一个蛛网", 
        flavor_text = "节肢动物，小子",
        ...
        behavior = {
            ...
            combat_unit.Attack:RegisterOnMissileImpactCallback(name, function(level, attack_data, criminal, victim)
                ...
                if attack_data.missile_id == "Missile_ZhuYan" then
                    for i = 1, level do
                        combat_unit.Attack:NewMissileAttack(victim, "Missile_ZhuWang")
...
Api:RegisterPerk("Perk_JiPo1", {
        id = "Perk_JiPo1",
        display_name = "急迫Ⅰ",
        description = "获得".."<color=#47b05d>".."5%</color>"..Api.GameMechanicTags.TAG.attack_speed.."并且在战斗结束后".."获得".."<color=#47b05d>".."1</color>"..Api.GameMechanicTags.TAG.armor,     --护甲
        flavor_text = "安全第一",
...
Api:RegisterPerk("Perk_JiPo2", {
        id = "Perk_JiPo2",
        display_name = "急迫Ⅱ",
        description = "获得".."<color=#47b05d>".."10%</color>"..Api.GameMechanicTags.TAG.attack_speed.."并且在战斗结束后".."获得".."<color=#47b05d>".."3</color>"..Api.GameMechanicTags.TAG.income,     --收入
        flavor_text = "经济第一",
        icon = DCEI.Texture("Perk_JiPo"),
        perk_type = "perk",
        rarity = "epic",
...
Api:RegisterPerk("Perk_JiPo3", {
        id = "Perk_JiPo3",
        display_name = "急迫Ⅲ ",
        description = "获得".."<color=#47b05d>".."15%</color>"
...
Api:RegisterPerk("Perk_NongFu", {
        id = "Perk_NongFu",
        display_name = "农夫帽",
        description = "你的"..TAG_perk_shiwu.."额外造成等同于".."<color=#47b05d>".."10%</color>"..Api.GameMechanicTags.TAG.energy_regen.."的伤害，"
        .."同时再给予".."<color=#47b05d>".."1</color>"..Api.GameMechanicTags.TAG.energy_regen, 
        flavor_text = "种田已经刻在我的基因里了",
        ...
        behavior = {
            ...
            combat_unit.Attack:RegisterOnMissileImpactCallback(name, function(level, attack_data, criminal, victim)
            if  attack_data.missile_id == "Missile_ZhuYan" or attack_data.missile_id == "Missile_MianBao" or
                attack_data.missile_id == "Missile_NiuRou" or attack_data.missile_id == "Missile_JXC" or 
...
Api:RegisterPerk("Perk_MuYang", {
            ...
            behavior = {
                ...
                combat_unit.Attack:RegisterOnMissileImpactCallback(name, function(level, attack_data, criminal, victim)
                if  attack_data.missile_id == "Missile_Zhu" or attack_data.missile_id == "Missile_JSZR" or
                    attack_data.missile_id == "Missile_HaiGui" or attack_data.missile_id == "Missile_HaiTun" or 
                    attack_data.missile_id == "Missile_DLKL" or attack_data.missile_id == "tudou" then
                        if attack_data.missile_id == "Missile_Zhu" then
                            for i = 1, level do
                        ...
                        elseif  attack_data.missile_id == "Missile_HaiGui" then 
                            for i = 1, level do
                                combat_unit:ModifyAttribute("JS_BaoShi", -1, false)
                                damage = 15
                                victim:Damage(damage,victim, {})
                        ...
                        elseif  attack_data.missile_id == "Missile_HaiTun" then  
                            for i = 1, level do
                                combat_unit:ModifyAttribute("JS_BaoShi", -1, false)
                                damage = 20
                                victim:Damage(damage,victim, {})
                        ...
                        elseif  attack_data.missile_id == "Missile_DLKL" then  
                            for i = 1, level do
                                combat_unit:ModifyAttribute("JS_BaoShi", -1, false)
                                damage = 60
                                victim:Damage(damage,victim, {})
...
--AI代码生成测试
--挖矿药水
Api:RegisterPerk("Perk_GongJiang", {
    id = "Perk_GongJiang",
    display_name = "挖矿药水",
    description = "当你的" .. TAG_perk_gongju .. "命中时，获得" .. "<color=#47b05d>".."1%</color>" .. Api.GameMechanicTags.TAG.attack_speed,
    flavor_text = "工欲善其事，必先利其器",
    icon = DCEI.Texture("Perk_GongJiang"),
    perk_type = "perk",
    rarity = "rare",
    }, function(combat_unit)
        local name = "Perk_GongJiang"
        combat_unit.Attack:RegisterOnMissileImpactCallback(name, function(level, attack_data, criminal, victim)
            -- 检查是否是工具类型子弹
            if attack_data.missile_id == "Missile_ZSJ" or attack_data.missile_id == "Missile_GongJian" then  -- 钻石剑是工具类型
                for i = 1, level do
                    combat_unit:ModifyAttribute("attack_speed", 0.01, false)
                end
            end
        end)
    end)
--剧毒箭
Api:RegisterPerk("Perk_JuDuJian", {
    id = "Perk_JuDuJian",
    display_name = "剧毒箭",
    description = "当" .. TAG_missile_jian .. "命中敌人时，每秒对敌人造成5伤害，持续3秒",
    flavor_text = "见血封喉",
    icon = DCEI.Texture("Perk_JuDuJian"),
    perk_type = "perk",
    rarity = "rare",
    behavior = {
        description = "剧毒伤害",
        buff_bar_icon = DCEI.Texture("Perk_JuDuJian"),
        modify_attributes = {
            health_per_second = -5,
        },
        duration = 3,
...
Api:RegisterPerk("Perk_ZhiLiaoJian", {
    id = "Perk_ZhiLiaoJian",
    display_name = "治疗箭",
    description = "当" .. TAG_missile_jian .. "命中敌人时，恢复自身" .. "<color=#47b05d>".."5</color>" .. "点生命值",
    flavor_text = "箭头涂抹了治愈药水，既能伤敌又能自愈",
    icon = DCEI.Texture("Perk_ZhiLiaoJian"),
    perk_type = "perk",
    rarity = "common",
    }, function(combat_unit)
    local name = "Perk_ZhiLiaoJian"
    combat_unit.Attack:RegisterOnMissileImpactCallback(name, function(level, attack_data, criminal, victim)
        if attack_data.missile_id == "Missile_Jian" then
            for i = 1, level do
                -- 恢复自身10点生命值
                combat_unit:ModifyCurrentHealth(5)
            end
        end
    end)
    end)
        ...
        if attack_data.missile_id == "Missile_TNT" then
            for i = 1, level do
                ...
                if random_effect == 1 then
                    for i = 1, level * 2 do
                    -- 效果1：造成等同于能量恢复的10%伤害，并给予1能量恢复
                    local energy_regen = criminal:GetBoundedAttribute("energy_per_second")
                    local damage = energy_regen * 0.1
                    victim:Damage(damage, victim, {})
                    criminal:ModifyAttribute("energy_per_second", 1, false)
                    end
                elseif random_effect == 2 then
                    -- 效果2：消耗1点饱食度并获得1点回血
                    local current_baoshidu = criminal:GetBoundedAttribute("JS_BaoShi")
                    if current_baoshidu > 0 then
                        for i = 1, level * 2 do
                        criminal:ModifyAttribute("JS_BaoShi", -1, false)
                        criminal:ModifyAttribute("health_per_second", 1, false)
                        end
                    end
                    ...
                    for i = 1, level * 2 do
                    -- 效果3：获得1%攻击速度
                    criminal:ModifyAttribute("attack_speed", 0.01, false)
                    end
                end
            end
        end
...
Api:RegisterPerk("Perk_Power_ZhuYan", {
        ...
        perk_type = "missile",
        rarity = "common",
        flavor_text = "一般我都直接当垃圾扔了".."\n".." {===="..TAG_perk_shiwu..TAG_relic_zuanshi.."====}",
        attacks = {
            Missile_ZhuYan = 1,
            },
        behavior = {
...
Api:RegisterPerk("Perk_Power_Ji", {
        ...
        behavior = {
        description = "能量恢复",
        buff_bar_icon = DCEI.Texture("Missile_Ji"),
        modify_attributes = {
        ...
        if  old_jishu == ji_quest_max then 
            ...
            local deck_size = #combat_unit.Attack:GetEmojiDeck()
            if randoms == 1 then 
            combat_unit.Attack:AddEmojiToDeck("Missile_Zhu", 1)
            combat_unit:PlayAwardEmojiAnimation("Missile_Zhu", 1)
            combat_unit.run_instance:ModifySavedPerkDataEmojis("Missile_Zhu", deck_size)
            elseif randoms == 2 then 
                combat_unit.Attack:AddEmojiToDeck("Missile_DLKL", 1)
                combat_unit:PlayAwardEmojiAnimation("Missile_DLKL", 1)
                local deck_size = #combat_unit.Attack:GetEmojiDeck()
                combat_unit.run_instance:ModifySavedPerkDataEmojis("Missile_DLKL", deck_size)
            elseif randoms == 3 then
                combat_unit.Attack:AddEmojiToDeck("Missile_HaiTun", 1)
                combat_unit:PlayAwardEmojiAnimation("Missile_HaiTun", 1)
                local deck_size = #combat_unit.Attack:GetEmojiDeck()
                combat_unit.run_instance:ModifySavedPerkDataEmojis("Missile_HaiTun", deck_size) 
            end   
        end     
        end)
        end)
...
Api:RegisterPerk("Perk_Power_Zhu", {
        ...
        behavior = {
        description = "能量恢复",
        buff_bar_icon = DCEI.Texture("Missile_Zhu"),
        modify_attributes = {
            health_per_second = 0,
              },
        duration = 999,
...
--面包
Api:RegisterMissile("Missile_MianBao", {
        id = "Missile_MianBao",
        display_name = "面包",
...
--牛肉
Api:RegisterMissile("Missile_NiuRou", {
            id = "Missile_NiuRou",
            display_name = "牛肉饼",
            tip = "造成1伤害，命中时获得".."<color=#47b05d>".."5</color>"..Api.GameMechanicTags.TAG.energy_regen,
            flavor_text = "新鲜的不能再新鲜了，仿佛还在跳动！".."\n".." {========="..TAG_perk_shiwu.."=========}",
            icon = DCEI.Texture("Missile_NiuRou"),
            ...
            local name = "Missile_NiuRou"
            combat_unit.Attack:RegisterOnMissileImpactCallback(name, function(level, attack_data, caster, target)
                if attack_data.missile_id == "Missile_NiuRou" then
                   combat_unit:ModifyAttribute("energy_per_second", 5, false)
                   combat_unit:ModifyAttribute("JS_BaoShi", 1, false)
                end
            end)
...
Api:RegisterMissile("Missile_JXC", {
                ...
                local name = "Missile_JXC"
                combat_unit.Attack:RegisterOnMissileImpactCallback(name, function(level, attack_data, caster, target)
                    if attack_data.missile_id == "Missile_JXC" then
                       combat_unit:ModifyAttribute("energy_per_second", 5, false)
                       combat_unit:ModifyAttribute("JS_BaoShi", 1, false)
                    end
                end)
...
--钻石剑
Api:RegisterMissile("Missile_ZSJ", {
        id = "Missile_ZSJ",
        display_name = "钻石剑",
        tip = "造成你所有属性之和的伤害".."\n".." {========="..TAG_perk_gongju.."========}",
        flavor_text = "吃我一记跳劈",
...
Api:RegisterMissile("Missile_YanJing", {
        ...
        local name = "Missile_YanJing"
        local jishu = 1
        combat_unit.Attack:RegisterOnMissileImpactCallback(name, function(level, attack_data, caster, target)
            if attack_data.missile_id == "Missile_YanJing" then
                if jishu == 3 then
                    for i = 1,level do
...
--鸡
Api:RegisterMissile("Missile_Ji", {
		id = "Missile_Ji",
		display_name = "鸡",
        tip = "不造成伤害,你的"..TAG_perk_shiwu.."每命中敌人150次，随机孵化一枚蛋（不可叠加）",
...
--猪
Api:RegisterMissile("Missile_Zhu", {
        id = "Missile_Zhu",
        display_name = "猪",
        ...
        local name = "Missile_Zhu"
        combat_unit.Attack:RegisterOnMissileImpactCallback(name, function(level, attack_data, caster, target)
        
            end)
        end)
--海龟
Api:RegisterMissile("Missile_HaiGui", {
    id = "Missile_HaiGui",
    display_name = "海龟",
    ...
    flavor_text = "“我表哥会大爆炸”，乔治一边啃着胡萝卜一边说着".."\n".." {========="..TAG_perk_shengwu.."=========}",
    icon = DCEI.Texture("Missile_HaiGui"),
    ...
    local name = "Missile_HaiGui"
    combat_unit.Attack:RegisterOnMissileImpactCallback(name, function(level, attack_data, caster, target)
        if attack_data.missile_id == "Missile_HaiGui" then
...
Api:RegisterMissile("Missile_HaiTun", {
        ...
        damage = function(attack_data, caster)
            ...
            local name = "Missile_HaiTun"
            combat_unit.Attack:RegisterOnMissileImpactCallback(name, function(level, attack_data, caster, target)
                local target = combat_unit:GetMainOpponent()
                if attack_data.missile_id == "Missile_HaiTun" then
...
--凋零骷髅
Api:RegisterMissile("Missile_DLKL", {
    id = "Missile_DLKL",
    display_name = "凋零骷髅",
    tip = "造成120点固定伤害",
    flavor_text = "力大砖飞".."\n".." {========="..TAG_perk_shengwu.."=========}",
    icon = DCEI.Texture("Missile_DLKL"),
...
--箭矢
Api:RegisterMissile("Missile_Jian", {
    id = "Missile_Jian",
    display_name = "箭矢",
    tip = "造成5点伤害",
    flavor_text = "箭在弦上，不得不发",
    icon = DCEI.Texture("Missile_Jian"),
    missile = DCEI.SimpleUnit("COMBAT Missile Jian"),
    sounds = {
        DCEI.Sound("jian")
    },
    damage = function(attack_data, caster)
...
Api:RegisterPerk("boss_luobo", {
    id = "boss_luobo",
    display_name = "胡萝卜",
    description = "获得"..TAG_missile_huluobo .. "\n" .. "造成1伤害，命中时获得".."<color=#47b05d>".."5</color>"..Api.GameMechanicTags.TAG.energy_regen,
...
Api:RegisterPerk("boss_tudou", {
    id = "boss_tudou",
    display_name = "马铃薯",
    description = "获得"..TAG_missile_tudou .. "\n" .. "造成1伤害，命中时获得".."<color=#47b05d>".."5</color>"..Api.GameMechanicTags.TAG.energy_regen,
...
Api:RegisterMissile("tudou", {
    ...
    display_name = "马铃薯",
    tip = "造成1伤害，命中时获得".."<color=#47b05d>".."5</color>"..Api.GameMechanicTags.TAG.energy_regen,
    flavor_text = "你接下来见证的是：人类文明食物来源官方合作伙伴，西伯利亚唯一官方指定作物，最多食用方式记录保持者，育碧EA服务器指定供应商",
    icon = DCEI.Texture("tudou"),
...
Api:RegisterPerk("boss_furou", {
    id = "boss_furou",
    display_name = "腐肉",
    description = "获得"..TAG_missile_furou .. "\n" .. "造成3%"..Api.GameMechanicTags.TAG.max_energy.."的伤害，同时降低对手5"..Api.GameMechanicTags.TAG.energy_regen.."持续5秒",
...
Api:RegisterPerk("boss_tieding", {
    id = "boss_tieding",
    display_name = "铁锭",
    description = "获得"..TAG_missile_tieding .. "\n".."造成双倍等同于"..Api.GameMechanicTags.TAG.armor.."的伤害".."每10次命中获得1"..Api.GameMechanicTags.TAG.armor,
...
Api:RegisterPerk("boss_changpian", {
    id = "boss_changpian",
    display_name = "唱片",
    description = "获得"..TAG_missile_changpian,
...
Api:RegisterPerk("boss_shazi", {
    id = "boss_shazi",
    display_name = "沙子",
    description = "获得"..TAG_missile_shazi.."\n命中时随机造成".."<color=#47b05d>".."10~20</color>".."伤害",
    icon = DCEI.Texture("shazi"),
    perk_type = "perk",
    rarity = "epic",
    flavor_text = "没人知道沙子里究竟藏了什么，TNT还是金子？".."\n".." {========="..TAG_relic_zuanshi.."=========}", 
    attacks = {
        shazi = 1,
        }, 
    })
...
Api:RegisterMissile("shazi", {
    id = "shazi",
    display_name = "沙子",
    tip = "造成".."<color=#47b05d>".."10~20</color>".."伤害",
    flavor_text = "",
    icon = DCEI.Texture("shazi"),
    ...
    damage = function(attack_data, caster)
            local randoms = math.random(10,20)
        return randoms
    end
    })
--可疑的沙子
Api:RegisterPerk("boss_keyishazi", {
    id = "boss_keyishazi",
    display_name = "可疑的沙子",
    description = "造成30伤害",
    icon = DCEI.Texture("shazi"),
...
Api:RegisterPerk("boss_jinding", {
    id = "boss_jinding",
    display_name = "金粒",
    description = "获得"..TAG_missile_jinli.."\n造成100伤害 命中时减少敌人1"..Api.GameMechanicTags.TAG.income,
    icon = DCEI.Texture("missile_jinding"),
    perk_type = "perk",
    rarity = "epic",
    flavor_text = "“积少成多，金粒也值不少钱呢，哼哼”".."\n".." {========="..TAG_relic_zuanshi.."=========}", 
    attacks = {
        missile_jinding = 1,
        }, 
    })
...
Api:RegisterMissile("missile_jinding", {
    id = "missile_jinding",
    display_name = "金粒",
    tip = "造成100伤害 命中时减少敌人1"..Api.GameMechanicTags.TAG.income,
    flavor_text = "蚊子再小也是肉，金粒也是金",
    icon = DCEI.Texture("missile_jinding"),
    ...
    damage = function(attack_data, caster)
        ...
        combat_unit.Attack:RegisterOnMissileImpactCallback(name, function(level, attack_data, caster, target)
            if attack_data.missile_id == "missile_jinding" then
                target.run_instance:ModifySavedPerkDataAttributes("income", -1)
            end
        end)
        end)
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------    
...
Api:RegisterPerk("shinaozhe", {
    id = "shinaozhe",
    display_name = "食脑者",
    description = "被脑子击中时降低".."<color=#47b05d>".."50</color>"..Api.GameMechanicTags.TAG.max_energy, 
    flavor_text = "其他游戏里同行的天赋，拿来用一下",
...
Api:RegisterPerk("taojin", {
    id = "taojin",
    display_name = "淘金者",
    description = "沙子击中敌人时有".."<color=#47b05d>".."15%</color>".."的概率额外发射一个金粒", 
...
Api:CopyCharacter("naysayer", {
        ...
        attributes = {
        health_maximum = 2100,          --hp
        energy_maximum = 2000,          --最大能量
